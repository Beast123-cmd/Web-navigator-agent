...
const AgentFlow = ({ isActive, currentStep }: AgentFlowProps) => {
  return (
    <Card className="p-4 sm:p-6 shadow-medium animate-fade-in-up">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          <div className="p-2 bg-gradient-primary rounded-md animate-glow">
            <Bot className="h-5 w-5 text-primary-foreground" />
          </div>
          <h3 className="text-base sm:text-lg font-semibold text-foreground">AI Agent Pipeline</h3>
        </div>
        <Badge variant="secondary" className={isActive ? "animate-pulse bg-primary text-primary-foreground" : ""}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.name} className="relative animate-fade-in-up" style={{ animationDelay: `${index * 0.1}s` }}>
              <div
                className={
                  "rounded-xl border p-3 sm:p-4 h-full transition-all duration-500 " +
                  (isCurrent
                    ? "border-primary shadow-glow bg-primary/10 scale-105"
                    : isDone
              ...
              {index < steps.length - 1 && (
                <div className={`hidden sm:block absolute top-1/2 right-[-10px] w-5 h-0.5 transition-colors duration-500 ${
